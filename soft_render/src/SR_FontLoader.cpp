...
-----------------------------------------------------------------------------*/
bool copy_glyph(SR_FontGlyph& pGlyph, const FT_GlyphSlot ftGlyph) {
...
/*-------------------------------------
    Move Constructor
-------------------------------------*/
SR_FontLoader::SR_FontLoader(SR_FontLoader&& f) :
    Resource{}
{
...
/*-------------------------------------
    Move operator
-------------------------------------*/
SR_FontLoader& SR_FontLoader::operator =(SR_FontLoader&& f) {
    unload();
...
-------------------------------------*/
void SR_FontLoader::unload() {
    // Re-cast to a glyph pointer to avoid a crash caused by CLang.
    delete [] reinterpret_cast<SR_FontGlyph*>(pData);
    pData = nullptr;
...
/*-------------------------------------
    Load a font file
-------------------------------------*/
bool SR_FontLoader::load_file(const std::string& filename, unsigned pixelSize) {
    FT_Library ftLib;
    FT_Face ftFace;
    FT_Error ftErr;
...
-------------------------------------*/
bool SR_FontLoader::load_glyphs(FT_FaceRec_* ftFace) {
    const unsigned ftNumGlyphs = (unsigned)ftFace->num_glyphs;
    SR_FontGlyph * const pGlyphs = new(std::nothrow) SR_FontGlyph[ftNumGlyphs];
...
/*-------------------------------------
    Save a file
-------------------------------------*/
bool SR_FontLoader::save_file(const std::string&) const {
    return false;
}
