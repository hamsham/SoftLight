...
#ifndef SR_TRANSFORM_HPP
#define SR_TRANSFORM_HPP
...
-----------------------------------------------------------------------------*/
enum SR_TransformType : uint32_t
{
    SR_TRANSFORM_TYPE_MODEL,
    SR_TRANSFORM_TYPE_VIEW_FPS, // should be default for all view types
    SR_TRANSFORM_TYPE_VIEW_ARC,
    SR_TRANSFORM_TYPE_VIEW_FPS_LOCKED_Y,
    SR_TRANSFORM_TYPE_VIEW_ARC_LOCKED_Y,
};
...
----------------------------------------------------------------------------*/
class alignas(sizeof(ls::math::vec4)*4) SR_Transform
{
  public:
    /**
  ...
  public:
     ...
     */
    SR_Transform(const SR_TransformType transformType = SR_TRANSFORM_TYPE_MODEL) noexcept;
     ...
     * Allows the transformation to be initialized to a specific type.
     */
    SR_Transform(const ls::math::mat4& modelMat, const SR_TransformType transformType = SR_TRANSFORM_TYPE_MODEL) noexcept;
     ...
     */
    SR_Transform(const SR_Transform& t) noexcept;
     ...
     */
    SR_Transform(SR_Transform&& t) noexcept;
     ...
     * A constant reference to another transform object
     */
    SR_Transform& operator=(const SR_Transform& t) noexcept;
     ...
     * An r-value reference to another transform object
     */
    SR_Transform& operator=(SR_Transform&& t) noexcept;
     ...
     *
     * @return A value from the SR_TransformType enumeration which can be
...
-------------------------------------*/
inline void SR_Transform::set_clean() noexcept
{
...
-------------------------------------*/
inline bool SR_Transform::is_dirty() const noexcept
{
...
-------------------------------------*/
inline void SR_Transform::set_dirty() noexcept
{
    mFlags |= transform_flags_t::TRANSFORM_FLAG_DIRTY;
}
 ...
 * Y-Axis Lock Check
-------------------------------------*/
inline bool SR_Transform::is_y_axis_locked() const noexcept
{
    return mType == SR_TRANSFORM_TYPE_VIEW_ARC_LOCKED_Y
...
-------------------------------------*/
inline void SR_Transform::look_at(const ls::math::vec3& target)
{
    look_at(mPosition, target, ls::math::vec3{0.f, 1.f, 0.f});
}


#endif /* SR_TRANSFORM_HPP */
